        ...
        if (nextProps.all_incidents !== null) {
          nextProps.all_incidents.map(incident => {
            ...
            if (
              ...
              parseInt(this.props.settings.notification_min_radius.toFixed(2))
            ) {
              if (
                new Date() -
                  new Date(
                    self.props.incident.notificationStack[incident.key].date,
                  ) >
                  this.props.settings.notification_timeout * 60 * 1000 ||
                self.props.incident.notificationStack[incident.key].isFirstTime
              ) {
